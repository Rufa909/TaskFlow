...
const searchUserByEmail = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query(
            'SELECT user_id, username, email, user_photo FROM users WHERE email = ? AND user_id != ?',
            [email, req.user.id]
        );
...
const sendInvitation = async (req, res) => {
    try {
        ...
        if (projects.length === 0) {
            return res.status(404).json({ success: false, message: 'Project không tồn tại hoặc bạn không phải owner' });
        }
        ...
        if (existingMembers.length > 0) {
            return res.status(400).json({ success: false, message: 'Người này đã là thành viên của project' });
        }
...
const respondInvitation = async (req, res) => {
    try {
        ...
        if (invitations.length === 0) {
            return res.status(404).json({ success: false, message: 'Lời mời không tồn tại hoặc đã được xử lý' });
        }
...
const getProjectMembers = async (req, res) => {
    try {
        ...
        const filteredMembers = members.filter(m => !ownerIds.includes(m.user_id));
